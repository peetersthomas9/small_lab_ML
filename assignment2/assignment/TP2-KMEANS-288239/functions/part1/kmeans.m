...
% while one of this two condition is not satified, continue to compute  a
% new centroid 
while has_converged==0
    
    % compute distance between centroid and data points ------------
    iter=iter+1; 
    Mu_previous=Mu;
    d =  distance_to_centroids(X, Mu, type);
    %----------------------------------------------------------------
    
    % compute responsability of each k-cluster : -----------------------------------------------
    [min_val labels]=min(d,[],1);
    r_i=zeros(K,N);
    for v=1:N
        r_i(labels(v),v)=1; 
    end
    ...
    check_empty=find(~sum(r_i,2));
    if isempty(check_empty)==0
        % reassign clusters randomly when one is empty 
        iter=0;
        Mu_init =  kmeans_init(X, K, init);
        Mu= Mu_init;
    % ----------------------------------------------------------------   
    else % if none is empty, compute the new centroid  Mu : 
        coeff=sum(r_i,2)*ones(1,D); % denominator
        Mu_trans=r_i*X.'./coeff; %(k,N)
        Mu=Mu_trans.'; %(N,k)
